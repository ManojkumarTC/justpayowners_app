...
const PropertyAmenitiesDetails = ({ tabItems, setSideNavTabs, isSale, isCommercial, islandorPlot }) => {
    ...
    useEffect(() => {
        if (params.tabtitle == "amenities") { // to get fresh data from server when tab is clicked
            fetchAdvartiseData(params.guid, userId).then((data) => {
                ...
                if (data != null && data.data != null && JSON.parse(data?.data?.propertyData).AmenitiesDetails) {
                    setAmenities(JSON.parse(data?.data?.propertyData).AmenitiesDetails);
                }
            });
        }
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        formData.propertyData = JSON.stringify({ ...propertyDataVal, ...localityObj });
